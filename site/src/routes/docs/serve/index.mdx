...
| `spa` | Serve a built SPA + the API from **one origin**: static assets, and an `index.html` fallback for client-side routes (`/download`, …). Mounted after the API, so `/api/*` always wins and unknown `/api/*` still 404s. |
