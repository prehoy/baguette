...
- [defineRoute](/docs/define-route/) — the full route contract: params, query, body, multi-status responses, tags, plus declarative **`auth`** and **`rateLimit`** (brute-force + email-bomb protection, built in).
- [serve](/docs/serve/) — configuring the server: security headers, the CORS credentials recipe, body limits, SPA serving, `onBoot`/`onShutdown`, typed `defineEnv`, and auto-wiring the optional layers.
